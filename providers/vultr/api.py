"""Vultr REST API wrapper (https://www.vultr.com/api/)."""
import time

import requests

from core.config import VULTR_API_KEY
...
def get_instance(instance_id):
    return _get(f"/instances/{instance_id}")["instance"]
...
def create_instance(region, plan, os_id, label, user_data, hostname=None, ssh_key_ids=None):
    ...
    return _post("/instances", payload)["instance"]
...
def list_regions():
    return _get("/regions").get("regions", [])
